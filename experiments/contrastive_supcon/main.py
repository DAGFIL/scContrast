...
Usage:
    python experiments/contrastive_supcon/main.py --run-name baseline --epochs 50
...
from viz import class_distribution_figure, similarity_histogram_figure, umap_figure
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--raw-path", default=str(RAW_DIR / "blood_homo_sapiens.h5ad"))
...
def evaluate_split(model, dataset, device, label_names, writer, tag, step, log_figure, seed,
    ...
    if log_figure:
        ...
        sim_fig = similarity_histogram_figure(embs, labels, title=f"{tag} pairwise cosine similarity (epoch {step})", seed=seed)
        writer.add_figure(f"similarity/{tag}", sim_fig, step)
    ...
    if log_projector:
        ...
        writer.add_embedding(
            embs[idx], metadata=[label_names[i] for i in labels[idx]],
            tag=f"projector_{tag}", global_step=step,
        )
    return m
...
def main() -> None:
    ...
    for split_name in ["train", "val", "test_seen", "test_unseen"]:
        ...
        if len(ds) == 0:
            continue
        names = [label_names[i] for i in ds.labels]
        fig = class_distribution_figure(names, title=f"{split_name} class distribution (n={len(ds):,})")
        writer.add_figure(f"class_distribution/{split_name}", fig, 0)
    ...
    test_seen_metrics = evaluate_split(model, datasets["test_seen"], device, label_names, writer, "test_seen", args.epochs, log_figure=True, seed=args.seed, log_projector=True)
    test_unseen_metrics = evaluate_split(model, datasets["test_unseen"], device, label_names, writer, "test_unseen", args.epochs, log_figure=True, seed=args.seed, log_projector=True)
